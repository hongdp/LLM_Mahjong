...
SYSTEM_PROMPT_NO_THINK = (
    ...
    "### 输出格式要求：\n"
    "你必须且只能从当前状态的【合法动作】列表中选择一个动作，原样复制输出。\n"
    "直接输出唯一的单行XML动作（如 <action type=\"discard\" tile=\"1m\" />），"
    "不要输出思考过程或任何其他内容。\n"
)


def get_system_prompt(no_think: bool = False) -> str:
    """think 审计（exp1）：2B 模型 think 内容 6-11% 正确且占生成 token 70-80%；
    no_think 变体是 rollout 2-3x 提速的消融开关（exp3_nothink）。"""
    return SYSTEM_PROMPT_NO_THINK if no_think else SYSTEM_PROMPT
