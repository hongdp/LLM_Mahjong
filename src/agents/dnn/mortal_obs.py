...
def _is_aka_slow(tile: str) -> bool:
    return tile.replace("*", "")[0] == "0"


# perf 2026-08-25: the encoder called _tid ~186x and _is_aka ~145x per
# observation, each doing a str.replace + int() parse -- together ~35% of
# encode time in a CPU-bound rollout. The tile alphabet is tiny and fixed, so
# both collapse into dict lookups built once at import. Values are identical
# by construction (the tables are generated by the reference functions).
_ALL_TILES = ([f"{r}{s}" for s in "mps" for r in range(0, 10)]
              + [f"{r}z" for r in range(1, 8)])
_TID = {}
_AKA = {}
for _t in _ALL_TILES:
    try:
        _TID[_t] = _tid_slow(_t)
        _AKA[_t] = _is_aka_slow(_t)
        _TID[_t + "*"] = _TID[_t]          # the engine's '*' marker variant
        _AKA[_t + "*"] = _AKA[_t]
...
def _tid(tile: str) -> int:
    v = _TID.get(tile)
    return v if v is not None else _tid_slow(tile)


def _is_aka(tile: str) -> bool:
    v = _AKA.get(tile)
    return v if v is not None else _is_aka_slow(tile)
...
class _Ctx:
    ...
    def __init__(self, planes: int, derived: bool):
        ...
        # perf 2026-08-25: writes are BUFFERED, not applied one at a time. The
        # encoder issues ~87 whole-row fills and ~116 single-cell assigns per
        # observation; as individual numpy calls that is ~200 x ~1us of call
        # overhead, which dominated a CPU-bound rollout. Buffering collapses
        # them into two vectorised scatters in `finish()`.
        #
        # Ordering: numpy fancy indexing keeps the LAST write to a duplicate
        # index, which matches sequential semantics. Fills are flushed before
        # assigns, which is only safe because no row is ever assigned and then
        # filled -- `test_mortal_obs_bit_identity` locks that in against a
        # golden set captured from the pre-optimisation implementation.
        self._fills = []          # (row, value)
        self._assigns = []        # (row, col, value)

    def fill(self, row: int, v: float = 1.0) -> None:
        self._fills.append((row, v))

    def assign(self, row: int, col: int, v: float = 1.0) -> None:
        self._assigns.append((row, col, v))

    def assign_rows(self, row: int, col: int, n: int, v: float = 1.0) -> None:
        for r in range(row, row + n):
            self._assigns.append((r, col, v))

    def finish(self) -> np.ndarray:
        arr = np.zeros((self.planes, TILE_TYPES), dtype=np.float32)
        if self._fills:
            f = np.asarray(self._fills, dtype=np.float64)
            arr[f[:, 0].astype(np.intp), :] = f[:, 1, None].astype(np.float32)
        if self._assigns:
            a = np.asarray(self._assigns, dtype=np.float64)
            arr[a[:, 0].astype(np.intp), a[:, 1].astype(np.intp)] = \
                a[:, 2].astype(np.float32)
        return arr

    # reads are rare (the fuuro block probes for a free channel); serve them
    # from the buffer so the semantics stay identical to direct writes
    def get(self, row: int, col: int) -> float:
        v = 0.0
        for r, c, val in self._assigns:
            if r == row and c == col:
                v = val
        return v
...
def _river_items(table: PyMahjongTable, pid: int, dora_tiles=None) -> List[dict]:
    ...
    """
    if dora_tiles is None:      # was recomputed once per seat = 4x redundant
...
def encode_mortal_obs(table: PyMahjongTable, player_id: int,
    ...
    _dora_set = {_tid(_dora_from_indicator(d)) for d in table.dora_indicators}
    kawa = {p: _river_items(table, p, _dora_set) for p in range(4)}
    mine = kawa[me]
    for k in range(6):
