...
|---|---|---|---|---|
| 2026-09-02 | `exp60_value_league_prereg`（running，本地冒烟） | 价值方法 v2（用户架构）：演员/学习者解耦 + 历史 ckpt 联赛 + 持久对局库（58 B/步） | 基建跑通：演员 CPU 20 worker 25 局/s、学习者 GPU 95% batch 2048、晋级评测（vs 上一代 + vs bc49 绝对刻度、连续两次 <0.48 冻结）；首次评测 0.464 held | ⏳ 等前两三代曲线定资源配比（演员回 GPU / 加演员）与消融（n-step、目标周期、历史混合） |
