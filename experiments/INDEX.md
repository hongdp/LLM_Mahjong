...
|---|---|---|---|---|
| 2026-09-03 | `exp62_suit_aug_prereg`（done，L40S Secure，$2.7） | 训练时花色置换增广 BC A/B：bc49 配方原样重跑（控制）vs `--suit_aug`（同缓存并行） | acc 0.8075 vs 0.8080（+0.05pp）；**A vs C 合并 20k 对 0.5025±0.0035**；破缺率 6.0%→2.8%；C vs bc49 0.5059、A vs bc49 0.5090（+2.6σ） | 增广无可测增益，花色对称线关闭；**同配方重跑噪声地板 ±0.5–0.9% share** → A/B 必须同批控制臂 + 多种子 |
| 2026-09-03 | `exp61_symmetry_prereg`（done，本机 0 元） | 花色置换对称性诊断：bc49 贪心在 6 个花色置换下的破缺率 + 测试时 6 视角对称平均评测 | 破缺率 **5.72%**（自摸 6.96%/鸣牌 1.32%），全部在 top-2 间隔 <1.0 的近平决策（≥1.0 的 8,322 个决策 0 破缺）；exp46I 6.83%；**对称平均 vs bc49 合并 20k 对 0.4998±0.0035**（A/A 0.500） | 测试时对称化零效应，不作部署形态；破缺率可作收敛诊断；增广价值只可能在训练样本效率 → 提议 exp62 BC 增广 A/B |
| 2026-09-02 | `exp60_value_league_prereg` 第 2 轮（done，云上 L40S×2 Secure，$4.3） | 分布式价值训练系统实测定设计：GPU 演员 pod → rsync → 学习者本地盘数据层 → 工作站拉取 + GCS；Double DQN bf16 b1024 | 88 分钟 / 87k 更新 / 203k 局 / 10 代晋级；**终评 n=4000：gen_0002 0.4993、gen_0008 0.4894、gen_0010 0.4859 vs bc49（A/A 0.500）**；lag 1–11s、换代 <10s、学习者 22k 样本/s、演员 62→34 局/s（池子 2→6）；对局库 399 分片/204k 局/12.4M 步/846MB 落主检出+GCS | 基建定型（设计文档 §5）；强度仍"能学不毁值不超越"且后期下滑；判据非传递（绝对 0.5075→0.4859 未触发冻结）→ 下一轮绝对刻度判据、每桌 ≤2 对手模型、1 学习者:2–3 演员、离线消融同库 |
| 2026-09-02 | `exp60_value_league_prereg` 第 1 轮（done，本地冒烟） | 价值方法 v2（用户架构）：演员/学习者解耦 + 历史 ckpt 联赛 + 持久对局库 | 基建全通（分片 68 B/步、演员 CPU 25 局/s、学习者 GPU 95% 11.7k 样本/s、晋级评测含 bc49 绝对刻度保险）；48 分钟 / 15k 更新 / 90k 局：4 次评测 0.467–0.488 全 held，**0 代晋级**；终评 candidate vs bc49 n=4000 = 0.4894±0.008；对局库 92k 局/5.5M 步/377MB 落盘+GCS | 基建成立、学习未起步；下一轮：演员回 GPU/加演员、同库并行消融（n-step/目标周期/历史混合/m）、配对牌山差分目标 |
| 2026-08-31 | `exp59_dqn_value_prereg`（done） | 价值方法线 v1：Double DQN on bc49 先验（单局，league 冻结 T=0 环境） | 七臂 + 正式 200k（$0.30）：有锚全部 0.49–0.50（v1.2 0.4985 / t1 0.4985 / v1.4 0.493 / v1.5 0.4901）、无锚 0.21、校准后去锚 0.447；头-only ≈ 全微调；两处训练器 bug 修复 | **能学、不毁值、不超越**：排序靠锚不靠 TD；病根=覆盖+样本效率 (σ/Δ)²，杠杆=配对牌山差分；后继 exp60 |
