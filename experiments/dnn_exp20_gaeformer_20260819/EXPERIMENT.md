...
- **Date**: 预注册 2026-08-19  **Status**: done（M1/M2 未达；梯度竞争假说证伪）
...
| **M2（1.2M 正式评分）** | **1057.6 ± 13.2** | bcrl14_600 1117.7；GAE 1079.7；自身 700k 1066.7 | >1117.7+2SE ⇒ **未达**；700k→1.2M **零净增** |
| 1.2M vs bcrl14_600（seed0=20260829） | −972 ± 1077（146:215） | — | 显著正 ⇒ **未达** |
| 1.2M vs GAE-700k（seed0=20260830） | +43 ± 1012（175:160）null | — | 合体未超 GAE 单臂 |
| 风格（4000 局） | 立直 **0.271** / 副露 0.366 / 放铳 0.161 | GAE 0.243/0.421 | ≥20% ✅（史上最人类化） |
| 饱和点防守探针 @1.2M | defense_iq **−0.003**；曝露放铳 0.217；V 跌 −1583/−1831/−2200 | — | **攻击已饱和、防守仍零 ⇒ 梯度竞争假说证伪** |
...
## Conclusion
1. **M1/M2 双未达**：GAE × ConvFormer 不可加——终点 1057.6 ≈ ConvFormer 单臂量级，低于 GAE 单臂
   1079.7；700k→1.2M 五十万局零净增（ladder 1035-1097 震荡收于 1058）。纯自对弈冠军仍是
   **exp17-C（cnn+GAE，1079.7）**；ConvFormer 谱系（含合体）平台 ~1060，与 vit 谱系的 ~930、
   cnn 无 GAE 的 ~1012 并列为三个明确的渐近线。
2. **梯度竞争假说证伪**：本 run 在 1.2M 已饱和（Elo 零增、熵 0.55 台阶后稳态），defense_iq 仍
   −0.003，而 critic 的威胁感知依旧强（V 跌 1.5-2.2k）。「攻击饱和后防守自发浮现」不成立。
   剩余解释=生态均衡（当前种群推牌近似最优，折牌无回报）→ **exp22 联赛升为主手术**；
   输入层假说由 exp23 裁决。
3. 风格继续向人类收敛（立直 27%/副露 37%）但强度不涨——风格 ≠ 强度（exp10 教训的又一例）。
4. 工程侧收获：合体 run 暴露了 CPU 逐次推理对大模型的吞吐墙（13.6 局/s），催生 rollout 引擎
   优化（已 1.8× 云端实测）与 GPU 批推理计划。
