...
5. **多模型托管（2026-08-22 晚，用户问「对手走 CPU 会不会拖慢 / 能否多模型上 GPU」）**：服务进程现托管
   「学习者 + 整个联赛池」，槽位携带 model_id，每批按模型分组前向（各模型独立 CUDA graph 桶，
   编码宽度各异：v1 15 平面 / v3 50 平面共用最宽缓冲）；`--gpu_infer_opponents` 开启。踩坑：多个
   shim 共用一个 worker 槽位时，per-object 的完成代计数会读到陈旧结果（返回别的模型的动作 →
   非法动作 KeyError）——改为请求时从槽位读代数。本地 smoke：三种模式 learner-only 轨迹均正确；
   小模型池在本机 CPU 路径更快（24 vs 12 局/s），大模型池（≥10M）时 GPU 托管才是必需。
6. 教训（入 SKILLS）：spawn 重导入 `__main__`（入口必须守卫）；`torch.nonzero` 不能碰并发写的共享
