...
4. **统计等价检验（判据 3）**：exp17c 权重、同 384 种子、16 workers：决出率 CPU 0.794 vs GPU 0.841
   （差 +0.047 ± 0.055，CI 内）、有立直局 0.497 vs 0.542、|回报| 均值 3.376 vs 3.393——两路在此功效下
   无显著差异（RNG 路径不同，非逐字节）。用户裁定：后续所有训练 run 走 `--gpu_infer`。
