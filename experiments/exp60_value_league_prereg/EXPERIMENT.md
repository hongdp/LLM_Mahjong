...
- [09-02 09:05–09:45] **gen_0004 → gen_0010**（21 次晋级评测，10 次晋级，11 次 held；完整表 `exp60_r2_learner/promotions.txt`）。
  相对判据几乎每次都在 0.49–0.51 之间掷硬币（n=2000 的 ±0.011），绝对刻度（vs bc49）从 gen_0002 的 0.5075 逐代
  滑到 0.4795–0.4995，最低 0.4698（@148k，held）。冻结规则（连续两次 <0.48）一次都没触发。
  **演员吞吐随池子变大衰减**：池子 2 → 6 个模型时演员 62 → 34 局/s（每 GPU 批里模型种类变多、每模型批变小），
  学习者摄入 48.7 → 38.4 局/s，有效回放比 8 → 学习者更饿。
- [09-02 09:50] **按预算窗口收摊**：演员 STOP、学习者 kill（291 迭代 / 87.3k 更新 / 202,752 局 / 88 分钟）、
  最后一次拉取（399 分片 / 204,288 局 / 12.37M 步 / 846MB，池子 gen_0001–0010）→ 主检出 + GCS，两台 pod 经 API
  **terminate（204）**。工作站侧 n=4000 对终评见下。

## Results（第 2 轮，云上 L40S×2，2026-09-02 08:16 → 09:50）

| 判据 | 目标 | 实测 | 判定 |
|---|---|---|---|
| 2 代际曲线不降 | 相对晋级链单调 | 10 代晋级，但**绝对刻度不单调**：0.5005 → 0.5075 → 0.4795 → … → 0.4858 | ❌（判据非传递，接受了下滑） |
| 3 任一代 vs bc49 n=4000 ≥0.52 | ≥0.52 | **gen_0002 0.4993±0.0079（−168 分/对）**；gen_0008 0.4894（−349）；gen_0010 0.4859（−492）；A/A 0.5000/0.0 | ❌ 最佳代打平，后期代 −1.3σ/−1.8σ |
| 基建 | 端到端跑通 | 演员→学习者 rsync lag 1–11s；权重回传 <10s；工作站拉取+GCS 镜像；心跳零报警；无崩溃/卡死 | ✅ |
| 预算 | ≤ $5 | ≈ $4.3（演员 L40S ~2.3h + 学习者 L40S ~1.8h × $0.99 + 两台卡死 pod ~14 分钟 + 盘） | ✅ |

学习者：bf16 batch 1024 **21–26 更新/s = 21–26k 样本/s**（首迭代 26.4k，稳态 22k），TD 0.168 → 0.0126，q_mean ≈ 0。
演员：L40S 26 worker **62 局/s（池子 2 模型）→ 34 局/s（池子 6 模型）**。

## Conclusion（第 2 轮）

**分布式基建按设计成立且已定型（见设计文档 §5）；强度结论与第 1 轮/exp59 一致——"能学、不毁值、不超越"，
且长跑后期轻微下滑。** 三个新事实：
1. **晋级判据必须以绝对刻度为主**。"对上一代 ≥0.5"在 n=2000 噪声下是掷硬币，10 代里至少 5 代是噪声晋级，
   绝对刻度从 0.5075 滑到 0.4859 而冻结规则从未触发。下一轮：晋级 = 相对 ≥0.5 **且** 绝对 ≥ 上一代绝对 − 1σ；
   或直接只用绝对刻度（bc49 是固定锚，传递性问题消失）。
2. **演员吞吐与池子多样性反相关**（62 → 34 局/s）。GPU 推理服务器按模型分批，池子里模型越多每批越小。
   设计上要么限制同桌对手模型数（K=2 而非 last-K + 全部锚点），要么演员按"每桌同一对手集合"分组凑批。
3. **一台 L40S 演员喂一台 L40S 学习者，有效回放比 8**——学习者一直在饿。按 §1 公式和 3090 secure 32 核的核数/价格，
   一学习者应配 2–3 台 L40S 或 1 台 32 核 3090 演员；本轮没加演员是因为预算上限。

**下一轮该做的**：①绝对刻度晋级判据（已列 TODO，改 `promotion_eval`）；②margin m 0.05 → 0.01（Q 负值/占位符问题，
见 designs/design_dqn_value.md 附录）；③配对牌山差分目标；④在已落地的 12.4M 步对局库（`exp60_r2_store`）上
**离线**做 n-step / target_every / history_frac 消融——0 元、可复现，比再烧演员便宜。
...
| **第 2 轮** experiments/exp60_r2_store/ + gs://llm-mahjong-experiments/exp60_r2_store/ | 846MB | 399 分片 / 204,288 局 / 12.37M 步（四席全收，来源标签）+ tensorboard_actor/ |
| experiments/exp60_r2_pool/ + gs://…/exp60_r2_pool/ | 90MB | anchors.json + gen_0001–gen_0010.pt + candidate.pt |
| experiments/exp60_r2_learner/ + gs://…/exp60_r2_learner/（GCS 不含 games_*.pt） | — | config.json、train_log.json、tensorboard/、learner_stdout.log、promotions.txt、games_*.pt 8 个 |
| experiments/probes/exp60_r2_gens_vs_bc49_n4000.json | 1KB | 第 2 轮终评（A/A + gen_0002/0008/0010） |
