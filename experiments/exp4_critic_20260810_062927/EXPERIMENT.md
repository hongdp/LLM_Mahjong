...
## 探针后记（2026-08-11，同日完成）
同一批 5918 状态上：LLM 隐状态→回报 EV **0.0215**；引擎真值 9 维特征→回报 EV **0.0208**；
隐状态→自家点数（正对照）EV **0.4606**。⇒ 探针有效、表征无罪、**回报本质不可预测**。
critic 路线的立项前提被推翻，退役而非暂缓。附带机制解释：引擎特征中 shanten/ukeire 权重
≈0 而自家点数权重 −0.61，说明手牌质量几乎不预测结算——PBRS 势函数建在向听/受入上，
exp2 两臂打平由此获得机制层解释。
