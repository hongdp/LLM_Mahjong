...
## Progress（追加）
- [14:35] 臂 1/2 完成：1004.4 / 1008.6，贪心 vs A −6477/−6040——两臂等差 ⇒ 共因。
- [14:50 归因修正（用户质询后）] 控制器**末端跟踪其实达标**（876k：目标 0.28 vs 实际 0.30；1M：0.25 vs 0.257），
  早期 α 衰减也是"H 高于目标时不加奖励"的正确行为，仅 250k–500k 有 ~0.1 下冲（增益/anti-windup 小问题）。
  真正的嫌疑是**目标曲线选得太低**：0.5 起始远低于自然熵平台（~1.0），两臂目标虽不同但早期都等效于
  "提前 350k 锐化"，故结果相同——比 exp28 已证有害的时间表更激进。最终归因待臂 5（冠军配方 × 纪元 4）：
  若臂 5 ≈1060 ⇒ 判"低目标曲线失败"，控制器可留用但目标须从自然平台上方缓降（如 1.0→0.4）；
  若臂 5 也 ≈1005 ⇒ 撤回对熵的全部指控，代价来自纪元 4 场随机化本身。
