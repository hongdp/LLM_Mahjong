...
- [23:25] 用户要求"马上切"，中途更新到最新 checkpoint（games 802,816/iter98 →
  **1,007,616/iter123**）；切换时正有真实对局进行中（135 次决策，日志刚更新）——按用户明确指令
  立即切，接受空窗期代价（同前次切换模式，预期 MC 会自动 resync）。晋级门进度 gen9→10：
  gate_share 0.44→0.5→0.515，仍未通过（趋势向上）。
