# exp5_confirm_20260811

- **Date**: 2026-08-11 预注册  **Status**: complete（4 场 exit 0，2026-08-12）
...
| 对局 | 副数 | 差分 | 95%CI | p | 判定 |
|---|---|---|---|---|---|
| **ep40 vs 锚点（主判据）** | 96 | +1093 | ±1205 | 0.076 | ❌ **未确认**（CI 含 0） |
| ep35 vs 锚点 | 48 | −65 | ±2426 | 0.96 | 探索性 |
| ep45 vs 锚点 | 48 | +760 | ±1869 | 0.43 | 探索性 |
| exp2-S ep25 vs 锚点 | 48 | +1271 | ±1392 | 0.074 | 探索性 |

## Conclusion
H1 未获确认：ep40 的 +2950 是胜者诅咒（5 场里的最大值），新种子上回落到 +1093。
**但汇总 7 个 checkpoint 后得到更好的模型**：exp4 平均 vs 锚点 +1076±753（p=0.0051），
异质性 Q=3.58/df=6 → 各 checkpoint 无实质差异。正确图景是「**平坦的 ~+1000 点增益**，
ep10 起即存在、之后不增长（趋势 p=0.36）」，而非任何单点尖峰。
这同时解释了 exp2-S 的 null：同一效应量下 32-64 副根本测不出来（需 ~139 副）。
完整分析：docs/report_exp5_confirm_20260812.md
