...
- 红五（#6 之后）：引擎已有赤宝牌，MJAI `5mr` ↔ 引擎 `0m`；影子桌按座位记红五数，编码器 `v1r/v3r` 带红五平面，
  `discard0/riichi0` 是独立动作（打红五 vs 打普通五由策略决定）；吃碰优先消耗普通牌，杠带上红五。
  旧（无红五）checkpoint 仍可用：`load_compatible` 把 272 维头拓宽到 374 维（红五行复制普通行 −1 logit），
  观测里红五按普通 5 处理。
- 九种九牌：引擎动作 `kyuushu` → MJAI `ryukyoku` 反应，MC 会点「流局」。

## 已知失真（不影响合法性，影响强度上限）
- **单局制**：引擎一局即终局，没有连庄 / 本场 / 半庄顺位压力，本场数不进观测（只影响策略，不影响合法性）。
- 西场（延长战）按南场编码。
- 用 #6 之前训练的 checkpoint（如 exp17-C）时：红宝牌价值不可见，观测把红五当普通 5。
- 不支持三麻。
- 规则层面其余已与雀魂对齐（#6）：赤宝牌、无人和、双倍役满、流局满贯、明杠/加杠宝牌后翻、
  九种九牌 / 四风连打 / 四家立直 / 四杠散了、三家和了流局。
